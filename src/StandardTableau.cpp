...
bool StandardTableau::applyTransposition(int t, int &distance) {
  ...
  distance =
      (bj - bi) - (aj - ai); // a very weird signed distance returned here...

  if (((ai == bi) && (aj + 1 == bj)) || ((aj == bj) && (ai + 1 == bi)))
    return false;
  // These are the only two ways that applying the transpisition could yield a
  // non-standard tableau.
  else {
    (*this)[bi][bj] = t;
    (*this)[ai][aj] = t + 1;
    return true;
