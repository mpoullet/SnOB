...
public:
  ...
  bool YORComputed;
  int YoungOrthogonalCoefficients(const int tau, const int T, double &c1,
                                  double &c2) const;
  // Return precomputed information on how the transposition (tau,tau+1) acts on
  // tableau T. The result is either a linear combination of T and another
  // tableau Tdash (whose index is returned by the function) or just a multiple
  // of T. In the latter case, the function returns -1. All tableaux are indexed
  // according to their index in the vector tableaux below. Results for c1 and
  // c2 returned in argument!!!
  void computeYOR();
