...
Sn::Irreducible::Irreducible(const Sn *_group, const Partition &_partition)
  ...
  for (vector<Partition>::iterator it = subpartitions.begin();
       it != subpartitions.end(); it++) {
    int subrepresentationindex; // warning: this is return argument of following
                                // function
    Irreducible *subrepresentation =
        group->subgroup->irreducible(*it, subrepresentationindex);
...
void Sn::Irreducible::computeYOR() {
  ...
  for (int T = 0; T < degree; T++) { // for each tableau...
    for (int j = 1; j < n; j++) {    // for each transposition (j,j+1)...
      ...
      tdash[index] =
          YoungOrthogonalCoefficients(j, T, coeff1[index], coeff2[index]);
    }
  }
  YORComputed = 1;
}
...
void Sn::Irreducible::applyCycleL(const int j, Matrix<FIELD> &M, int m,
                                  bool inverse) {
  bool done[degree];
  FIELD *array = M.array;
  if (!YORComputed)
...
void Sn::Irreducible::applyCycleR(const int j, Matrix<FIELD> &M, int m,
                                  bool inverse) {
  bool done[degree];
  FIELD *array = M.array;
  if (!YORComputed)
