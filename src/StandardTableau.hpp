...
public:
  ...
  // bool operator==(const StandardTableau& t) const {return r==t.r;} // Test
  // for identical entries bool shapeequiv(const StandardTableau& t) const
  // {return p==t.p;} // Test for identical shape bool shapeequiv(const
  // Partition& _p) const {return p==_p;}       // Test if tableau is of shape
  // _p
  ...
  // Apply transposition (t,t+1) to tableau. If result is a standard tableau,
  // return true. Change distance to the special signed distance needed for
  // constructing Young's orthogonal representation. Result returned in
  // argument!!!
